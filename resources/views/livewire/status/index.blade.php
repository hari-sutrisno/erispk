    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="max-w-full overflow-x-auto p-4">
            ...
            <div
                    ...
                    <thead class="bg-gray-100 dark:bg-white/[0.04]">
                        <tr>
                            ...
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('tblm_status.status_code')">
                                Status Code {!! $this->sortField === 'tblm_status.status_code'
                            ...
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('tblm_status.keterangan')">
                                Keterangan {!! $this->sortField === 'tblm_status.keterangan'
                                    ? ($this->sortDirection === 'asc'
                                        ? ' &#129057;'
                                        : ' &#129059;')
                                    : '' !!}
                            ...
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('tblm_status.created_at')">
                                Created {!! $this->sortField === 'tblm_status.created_at'
                                    ? ($this->sortDirection === 'asc'
                                        ? ' &#129057;'
                                        : ' &#129059;')
                                    : '' !!}
                            ...
                            @forelse ($statuses as $status)
                                <tr wire:key="row-{{ $status->id }}">
                                    <td class="px-4 py-2 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ $statuses->firstItem() + $loop->index }}</td>
                                    <td class="px-4 py-2 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ $status->status_code }}</td>
                                    <td class="px-4 py-2 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ $status->keterangan }}</td>
                                    <td class="px-4 py-2 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ $status->created_at->format('d-m-Y') }}</td>
                                    <td class="px-4 py-2 text-end space-x-2">
                                        ...
                                        @can('status delete')
                                            <x-tooltip content="Delete" position="top">
                                                <button type="button"
                                                    x-on:click="$dispatch('confirm-delete', { id: {{ $status->id }} })"
                ...
                {{-- Pagination --}}
                @if ($this->ready && method_exists($statuses, 'links'))
