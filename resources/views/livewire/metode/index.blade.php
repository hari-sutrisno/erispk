        ...
        <div class="max-w-full overflow-x-auto p-4">
            ...
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('metode.create') }}"
                    class="inline-flex items-center gap-1 px-3 py-2 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
            ...
            <div
                ...
                <div class="flex-1 min-w-[250px]">
                    ...
                    <x-text-input type="text" id="search" placeholder="Search by name"
                    ...
                    <thead class="bg-gray-100 dark:bg-white/[0.04]">
                        <tr>
                            ...
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('tblm_kategori.nama')">
                                Nama {!! $this->sortField === 'tblm_kategori.name'
                            ...
                            <th
                                class="px-4 py-2 cursor-pointer text-center text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                Status</th>
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('tblm_kategori.created_at')">
                                Created {!! $this->sortField === 'tblm_kategori.created_at'
                                    ? ($this->sortDirection === 'asc'
                                        ? ' &#129057;'
                                        : ' &#129059;')
                                    : '' !!}
                    ...
                    <tbody>
                        ...
                        @if ($this->ready)
                            @forelse ($metodes as $metode)
                                <tr wire:key="row-{{ $metode->id }}">
                                    <td class="px-4 py-2 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ $metodes->firstItem() + $loop->index }}</td>
                                    <td class="px-4 py-2 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ $metode->nama }}</td>
                                    <td class="px-4 py-2 text-center">
                                        <span
                                            class="inline-flex items-center justify-center gap-1 rounded-full px-3 py-0.25 text-sm font-medium text-white
               {{ $metode->status ? 'bg-success-500' : 'bg-error-500' }}">
                                            {{ $metode->status ? 'Aktif' : 'Nonaktif' }}
                                        </span>
                                    </td>

                                    <td class="px-4 py-2 text-theme-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ $metode->created_at->format('d-m-Y') }}</td>
                                    <td class="px-4 py-2 text-end space-x-2">
                                        ...
                                        @can('metode delete')
                                            <x-tooltip content="Delete" position="top">
                                                <button type="button"
                                                    x-on:click="$dispatch('confirm-delete', { id: {{ $metode->id }} })"
