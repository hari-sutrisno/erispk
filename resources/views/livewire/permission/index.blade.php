        ...
        <div class="max-w-full overflow-x-auto p-4">
            ...
            <div
                ...
                <div class="flex-1 min-w-[250px]">
                    ...
                    <x-text-input type="text" id="search" placeholder="Search by name or guard name"
            ...
            <div
                    ...
                    <thead class="bg-gray-100 dark:bg-white/[0.04]">
                        <tr>
                            ...
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('permissions.name')">
                                Name {!! $this->sortField === 'permissions.name' ? ($this->sortDirection === 'asc' ? ' &#129057;' : ' &#129059;') : '' !!}
                            </th>
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('permissions.email')">
                                Guard Name {!! $this->sortField === 'permissions.email' ? ($this->sortDirection === 'asc' ? ' &#129057;' : ' &#129059;') : '' !!}
                            </th>
                            <th class="px-4 py-2 cursor-pointer text-start text-theme-xs font-medium text-gray-500 dark:text-gray-400"
                                wire:click="sortBy('permissions.created_at')">
                                Created {!! $this->sortField === 'permissions.created_at'
                                ...
                                <tr wire:key="row-{{ $permission->id }}">
                                    ...
                                    <td class="px-4 py-2 text-end space-x-2">
                                        ...
                                        <x-tooltip content="Delete" position="top">
                                            <button type="button"
                                                x-on:click="$dispatch('confirm-delete', { id: {{ $permission->id }} })"
                ...
                {{-- Pagination --}}
                @if ($this->ready && method_exists($permissions, 'links'))
