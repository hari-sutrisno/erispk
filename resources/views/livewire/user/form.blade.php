            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800 mt-4">
                <form wire:submit.prevent="save" class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input wire:model.defer="email" type="text" id="email" placeholder="Enter Email"
                    ...
                    <div>
                        ...
                        <x-password-input wire:model.defer="password" placeholder="Enter password" :has-error="$errors->has('password')" />
                    ...
                    <div>
                        ...
                        @forelse ($roleOptions as $key => $roleName)
                            @php
                                $checkboxId = 'checkboxRole_' . $key;
                            @endphp

                            <label for="{{ $checkboxId }}"
                                class="flex cursor-pointer items-center text-sm font-medium text-gray-700 select-none dark:text-gray-400 mb-3">

                                <div class="relative">
                                    {{-- Checkbox hidden (peer) --}}
                                    <input type="checkbox" id="{{ $checkboxId }}" value="{{ $roleName }}"
                                        wire:model="roles" class="sr-only peer" @checked(in_array($roleName, $roles)) />
